...
void SecureMineralsIncome(Builder* builder_) {
  if (gAPI->observer().CountUnitType(sc2::UNIT_TYPEID::TERRAN_SCV) < 80) {
    ...
    for (const auto& i : town_halls()) {
      if (!i->orders.empty() || !i->IsBuildFinished()) continue;

      if (i->assigned_harvesters > i->ideal_harvesters) continue;
...
void CallDownMULE() {
  ...
  for (const auto& i : orbitals()) {
    if (i->energy < mule_energy_cost) continue;

    const sc2::Unit* mineral_target = units.GetClosestUnit(i->pos);
    if (!mineral_target) continue;

    gAPI->action().Cast(*i, sc2::ABILITY_ID::EFFECT_CALLDOWNMULE,
                        *mineral_target);
  }
}
...
const Expansion* GetBestMiningExpansionNear(const sc2::Unit* unit_) {
  ...
  for (auto& i : gHub->GetExpansions()) {
    if (i.owner != Owner::SELF) continue;

    const sc2::Unit* th = gAPI->observer().GetUnit(i.town_hall_tag);
    if (!th->IsBuildFinished()) continue;
...
void DistrubuteMineralWorker(const sc2::Unit* unit_) {
  ...
  if (expansion) target_loc = expansion->town_hall_location;

  auto patches = gAPI->observer().GetUnits(sc2::IsVisibleMineralPatch(),
                                           sc2::Unit::Alliance::Neutral);
  const sc2::Unit* mineral_target = patches.GetClosestUnit(target_loc);

  if (!mineral_target) return;
...
void Miner::OnStep(Builder* builder_) {
  ...
  if (gHub->GetCurrentRace() == sc2::Race::Terran) CallDownMULE();
}
