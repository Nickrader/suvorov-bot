...
void RepairMan::OnStep(Builder*) {
  if (gHub->GetCurrentRace() != sc2::Race::Terran ||
      m_damaged_buildings.empty() || gAPI->observer().GetMinerals() == 0)
    return;
  ...
  if (m_damaged_buildings.empty()) return;

  auto repairers = gAPI->observer().GetUnits(IsRepairer());

  if (repairers().size() >= 3) return;

  AssignRepairTask(*m_damaged_buildings.front());
}

void RepairMan::OnUnitDamaged(const sc2::Unit* unit_, float health_, float,
                              Builder*) {
  if (gHub->GetCurrentRace() != sc2::Race::Terran ||
      !sc2::IsBuilding()(*unit_) ||
...
void RepairMan::OnUnitDestroyed(const sc2::Unit* unit_, Builder* builder_) {
  ...
  if (!unit_->IsBuildFinished()) return;
