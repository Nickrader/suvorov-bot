...
SortByDistance::SortByDistance(const sc2::Point3D& point_) : m_point(point_) {}

bool SortByDistance::operator()(const Expansion& lhs_,
                                const Expansion& rhs_) const {
...
void Hub::OnUnitDestroyed(const sc2::Unit& unit_) {
  if (sc2::Unit::Alliance::Self) {
    switch (unit_.unit_type.ToType()) {
      case sc2::UNIT_TYPEID::PROTOSS_PROBE:
      case sc2::UNIT_TYPEID::TERRAN_SCV:
      case sc2::UNIT_TYPEID::ZERG_DRONE: {
        if (m_free_workers.Remove(Worker(unit_))) return;
...
const Cache<GameObject>& Hub::GetLarvas() const { return m_larva; }

const Expansions& Hub::GetExpansions() const { return m_expansions; }
...
Expansion* Hub::GetNextExpansion() {
  ...
  if (it == m_expansions.end()) return nullptr;
...
Expansion* Hub::GetExpansionOfTownhall(const sc2::Unit& unit_) {
  auto it = std::find_if(
      m_expansions.begin(), m_expansions.end(), [unit_](const Expansion& e) {
