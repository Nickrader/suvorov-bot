...
bool IsWorkerUnit::operator()(const sc2::Unit& unit_) const {
  switch (unit_.unit_type.ToType()) {
...
bool IsFoggyResource::operator()(const sc2::Unit& unit_) const {
  if (!sc2::IsGeyser()(unit_) && !sc2::IsMineralPatch()(unit_)) return false;
...
bool IsRefinery::operator()(const sc2::Unit& unit_) const {
  if (!unit_.IsBuildFinished()) return false;
...
bool IsGasWorker::operator()(const sc2::Unit& unit_) const {
  if (!sc2::IsWorker()(unit_)) return false;

  if (unit_.orders.empty()) return false;

  if (unit_.orders.front().ability_id == sc2::ABILITY_ID::HARVEST_RETURN) {
    if (unit_.buffs.empty()) return false;

    return unit_.buffs.front() ==
...
bool IsIdleTownHall::operator()(const sc2::Unit& unit_) const {
  return sc2::IsTownHall()(unit_) && unit_.orders.empty() &&
         unit_.IsBuildFinished();
}
