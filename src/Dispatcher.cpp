...
Dispatcher::Dispatcher(const std::string& opponent_id_)
  ...
  gAPI.reset(new API::Interface(Actions(), Control(), Debug(), Observation(),
                                Query()));
  m_plugins.reserve(10);

  if (opponent_id_.empty()) return;
...
void Dispatcher::OnGameStart() {
  ...
  gZapp.reset(new Zapp());
  gZapp->OnGameStart(m_builder.get());
  ...
  if (current_race == sc2::Race::Protoss) {
    ...
    // do nothing, that is the race we are screwing with
  ...
  for (const auto& i : m_plugins) i->OnGameStart(m_builder.get());
}
...
void Dispatcher::OnGameEnd() {
  ...
  for (const auto& i : m_plugins) i->OnGameEnd();
}
...
void Dispatcher::OnStep() {
  ...
  gHub->OnStep();
  gZapp->OnStep(m_builder.get());
  for (const auto& i : m_plugins) i->OnStep(m_builder.get());
...
void Dispatcher::OnUnitCreated(const sc2::Unit* unit_) {
  ...
  if (unit_->display_type == sc2::Unit::DisplayType::Placeholder) {
    gHistory.info() << "Placeholder of "
                    << sc2::UnitTypeToName(unit_->unit_type)
                    << " was detected\n";
    return;
  }

  gHistory.info() << sc2::UnitTypeToName(unit_->unit_type) << "(" << unit_->tag
                  << ") was created\n";

  gHub->OnUnitCreated(*unit_);
  gZapp->OnUnitCreated(unit_, m_builder.get());

  for (const auto& i : m_plugins) i->OnUnitCreated(unit_, m_builder.get());
}
...
void Dispatcher::OnUnitIdle(const sc2::Unit* unit_) {
  ...
  gZapp->OnUnitIdle(unit_, m_builder.get());

  for (const auto& i : m_plugins) i->OnUnitIdle(unit_, m_builder.get());
}

void Dispatcher::OnUnitDamaged(const sc2::Unit* unit_, float health_,
                               float shields_) {
...
void Dispatcher::OnUnitDestroyed(const sc2::Unit* unit_) {
  ...
  gZapp->OnUnitDestroyed(unit_, m_builder.get());

  for (const auto& i : m_plugins) i->OnUnitDestroyed(unit_, m_builder.get());
}

void Dispatcher::OnUpgradeCompleted(sc2::UpgradeID id_) {
  gHistory.info() << sc2::UpgradeIDToName(id_) << " completed\n";

  for (const auto& i : m_plugins) i->OnUpgradeCompleted(id_);
}

void Dispatcher::OnUnitEnterVision(const sc2::Unit* unit_) {
  gHub->OnUnitEnterVision(*unit_);
  gZapp->OnUnitEnterVision(unit_, m_builder.get());

  for (const auto& i : m_plugins) i->OnUnitEnterVision(unit_, m_builder.get());
}
